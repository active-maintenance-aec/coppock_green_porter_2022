# coppock_green_porter_2022/download_original.R
...
#   against. md5_published is the checksum Dataverse displays. Here all fourteen agree,
#   but they do not always: other deposits in this program carry published checksums
#   that verify neither the original nor the derived tabular file, so verification runs
#   against md5_served and any disagreement is reported.
#
#   This deposit puts every file inside a `replication_archive` directory label, which
#   the file names in the manifest carry, so the local tree reproduces the deposit's
#   layout and the paths in the maintained scripts resolve.
...
dataset_doi <- "doi:10.7910/DVN/UH2NQW"
...
walk(
  unique(manifest$directory_label),
  \(d) dir.create(here::here("original", d), showWarnings = FALSE, recursive = TRUE)
)
